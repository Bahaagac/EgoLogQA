...
### Blur computation contract (locked for comparability)
- grayscale conversion: `cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)`
- no resize and no normalization before blur calculation
- blur ROI margin from `thresholds.blur_roi_margin_ratio`
- ROI fallback: full frame when cropped ROI would be empty
- blur value: `cv2.Laplacian(gray_roi, cv2.CV_64F).var()`
- blur decision uses fixed threshold only:
  - `blur_ok = blur_value >= thresholds.blur_threshold_min`
- `blur_p10/p50/p90` are diagnostics only and do not affect blur pass/fail logic
- `blur_threshold_min=80.0` is tuned for 1080p MicroAGI00-style streams and may need retuning at different resolutions
...
- blur CSV and depth CSV when debug export is enabled
- blur fail/pass evidence frames when forced export is enabled or blur WARN auto-export triggers
- all artifact paths stored in report metrics are output-relative POSIX strings
...
- blur/depth CSV export enabled by default
- blur evidence export disabled by default, with auto-export-on-blur-WARN enabled
...
Pixel (RGB):
- blur:
  - `blur_median`, `blur_threshold`, `blur_fail_ratio`
  - `blur_p10`, `blur_p50`, `blur_p90`
  - `blur_valid_frame_count`
  - `blur_debug_csv_path`
  - `blur_fail_frames_dir`, `blur_pass_frames_dir`
- exposure:
  - `exposure_bad_ratio`
  - `exposure_valid_frame_count`
  - `exposure_bad_first_sample_i`, `exposure_bad_last_sample_i`
